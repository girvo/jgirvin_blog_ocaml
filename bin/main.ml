...
let () =
  ...
  if String.equal !input_dir !output_dir then
    fail "input and output dirs can't be equal";
  if not (is_valid_input_dir !input_dir) then
    fail "input directory must contain posts/ pages/ templates/ subdirectories";
  if not (is_valid_output_dir !output_dir) then
    fail "output directory must exist and be a directory";
